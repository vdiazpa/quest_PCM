'''Alternate version of EGRET's services.py. 
This module has been updated to include participation of ESS in ancillary services'''
from pyomo.environ import *
import math
from egret.model_library.unit_commitment.uc_utils import add_model_attr, uc_time_helper, make_penalty_rule
from egret.model_library.unit_commitment.status_vars import _is_relaxed
from .storage_params import StorageParams
from .storage_vars import StorageVars
from .storage_constraints import StorageConstraints

@add_model_attr('storage_service', requires = {'data_loader': None,
                                            })
def storage_services(model):
    """
    Defines storage services and constraints for all storage types (GESS, BESS, PHS) in the model.

    This function:
        - Initializes storage parameters, variables, and constraints for each storage type.
        - Adds expressions for storage costs and power input/output aggregation.
        - Enables participation of storage in ancillary services.

    Args:
        model: Pyomo model object to which storage services, variables, and constraints are added.

    Side Effects:
        Modifies the model in-place by adding parameters, variables, constraints, and expressions
        related to storage operation and ancillary service participation.

    Returns:
        None
    """
    # Initialize common storage parameters, variables, and constraints
    storage_params = StorageParams(model)
    storage_params.initialize_storage_common_params()

    storage_vars = StorageVars(model)
    storage_vars.define_storage_common_vars()

    storage_constraints = StorageConstraints(model)
    storage_constraints.enforce_common_storage_constraints()

    # Add GESS-specific constraints if present
    if value(model.GESS_present):
        storage_constraints.enforce_GESS_storage_constraints()

    # Add BESS-specific parameters, variables, and constraints if present
    if value(model.BESS_present):
        storage_params.initialize_BESS_params()
        storage_vars.define_BESS_vars()
        storage_constraints.enforce_BESS_constraints()

    # Add PHS-specific parameters, variables, and constraints if present
    if value(model.PHS_present):
        storage_params.initialize_PHS_params()
        storage_vars.define_PHS_vars()
        storage_constraints.enforce_PHS_unit_constraints()
        storage_constraints.enforce_PHS_constraints()
        
    # Define storage cost expression (includes slack and technology-specific costs)
    def storage_cost_rule(m, s, t): 
        if s in m.PHS_Storage:
            approx_MWh_cap = m.PHS_conversion_coefficient[s] * m.PHS_UpperReservoirMaxLevel[s]
        else:
            approx_MWh_cap = m.MaximumEnergyStorage[s]
        cost = m.soc_slack[s,t] * m.RT_SOC_deviation_penalty * approx_MWh_cap
        if s in m.GESS_Storage:
            cost += m.GESS_cost[s, t] 
        if s in m.BESS_Storage:
            cost += m.BESS_cost[s, t] 
        if s in m.PHS_Storage:
            cost += m.PHS_cost[s, t] 
        return cost
    ...
    # Helper function to aggregate power variables across storage types
    def aggregate_power(m, s, t, input_name_GESS, input_name_BESS, input_name_PHS):
        temp_val = 0
        if input_name_GESS and s in m.GESS_Storage:
            temp_val  = getattr(m, input_name_GESS)[s, t]
        if input_name_BESS and s in m.BESS_Storage:
            temp_val  = getattr(m, input_name_BESS)[s, t]
        if input_name_PHS and s in m.PHS_Storage:
            temp_val  = getattr(m, input_name_PHS)[s, t]
        return temp_val

    # Define expressions for aggregated storage power and ancillary service provision
    model.PowerInputStorage = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, "PowerChargeGESS", "BESS_TotalInput", "PHS_TotalInput")
    )
    model.PowerOutputStorage = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, "PowerDischargeGESS", "BESS_TotalOutput", "PHS_TotalOutput")
    )
    model.OutputStorage_RegUP = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, None, "BESS_RegUP", "PHS_TotalRegUP")
    )
    model.InputStorage_RegDOWN = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, None, "BESS_RegDOWN", "PHS_TotalRegDOWN")
    )
    model.OutputStorage_SP = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, None, "BESS_SP", "PHS_TotalSP")
    )
    model.OutputStorage_NSP = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, None, "BESS_NSP", "PHS_TotalNSP")
    )
    model.OutputStorage_SUPP = Expression(
        model.Storage, model.TimePeriods, 
        rule=lambda m, s, t: aggregate_power(m, s, t, None, "BESS_SUPP", "PHS_TotalSUPP")
    )
...
def ancillary_services(model):
    ...
    reg_up = _check_for_requirement('regulation_up_requirement')
    reg_down = _check_for_requirement('regulation_down_requirement')

    # validation
    if reg_up ^ reg_down:   # XOR → True if exactly one is True
        raise ValueError("If you want to enable regulation, both 'regulation_up_requirement' and "
    "'regulation_down_requirement' must be present.")

    add_regulation_reserve = reg_up and reg_down
    add_supplemental_reserve = _check_for_requirement('supplemental_reserve_requirement')

    flex_up = _check_for_requirement('flexible_ramp_up_requirement')
    flex_down = _check_for_requirement('flexible_ramp_down_requirement')

    if flex_up ^ flex_down:   # XOR → True if exactly one is True
        raise ValueError("If you want to enable flexible ramping, both 'flexible_ramp_up_requirement' and "
    "'flexible_ramp_down_requirement' must be present.")

    add_flexi_ramp_reserve = (flex_up and flex_down) 
    ...
    def storage_in_reserve_zone_getter(sto_attrs_subset=None):
        if sto_attrs_subset is None:
            sto_attrs = storage_attrs
        else:
            sto_attrs = sto_attrs_subset
        def get_storage_in_reserve_zone(m, az):
            az_n = str(az)
            if az_n[:5] == 'zone_':
                z_n = az_n[5:]
                for g in sto_attrs['names']:
                    if g in sto_attrs['area'] and sto_attrs['zone'][g] == z_n:
                        yield g
            elif az_n[:5] == 'area_':
                a_n = az_n[5:]
                for g in sto_attrs['names']:
                    if g in sto_attrs['area'] and sto_attrs['area'][g] == a_n:
                        yield g
            else:
                raise Exception('Unexpected case in instance of storage_in_reserve_zone_getter')
        return get_storage_in_reserve_zone
    
    ## these need to be added by high-quality to low-quality,
    ## except flexiramp which is it's own thing
    if add_regulation_reserve:
        regulation_services(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, storage_in_reserve_zone_getter)
    else:
        model.regulation_service = None
        _ = Constraint(model.Storage, model.TimePeriods, rule=lambda m, s, t: m.InputStorage_RegDOWN[s, t] == 0)
        _ = Constraint(model.Storage, model.TimePeriods, rule=lambda m, s, t: m.OutputStorage_RegUP[s, t] == 0)
        
                
    if add_spinning_reserve:
        spinning_reserves(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, thermal_gen_attrs, storage_in_reserve_zone_getter)
    else:
        model.spinning_reserve = None
        _ = Constraint(model.Storage, model.TimePeriods, rule=lambda m, s, t: m.OutputStorage_SP[s, t] == 0)

    if add_non_spinning_reserve:
        non_spinning_reserves(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, storage_in_reserve_zone_getter)
    else:
        model.non_spinning_reserve = None
        _ = Constraint(model.Storage, model.TimePeriods, rule=lambda m, s, t: m.OutputStorage_NSP[s, t] == 0)
                
    if add_supplemental_reserve:
        supplemental_reserves(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, thermal_gen_attrs, storage_in_reserve_zone_getter)
    else:
        model.supplemental_reserve = None
        _ = Constraint(model.Storage, model.TimePeriods, rule=lambda m, s, t: m.OutputStorage_SUPP[s, t] == 0)
...
def regulation_services(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, storage_in_reserve_zone_getter):
    ...
    model.StorageInRegulationZone = Set(model.RegulationZones, initialize=storage_in_reserve_zone_getter())
    ...
    def zonal_reg_dn_provided(m, rz, t):
        return (sum(m.RegulationReserveDn[g,t] for g in m.AGC_GeneratorsInRegulationZone[rz]) + m.ZonalRegulationDnShortfall[rz,t])
    model.ZonalRegulationDnProvided = Expression(model.RegulationZones, model.TimePeriods, rule=zonal_reg_dn_provided)
    
    def enforce_zonal_reg_up_requirement_rule(m, rz, t):
        return  m.ZonalRegulationUpProvided[rz,t] + sum(m.OutputStorage_RegUP[s,t] for s in m.StorageInRegulationZone[rz]) >= m.ZonalRegulationUpRequirement[rz,t]
    ...
    def enforce_zonal_reg_dn_requirement_rule(m, rz, t):
        return  m.ZonalRegulationDnProvided[rz,t] + sum(m.InputStorage_RegDOWN[s,t] for s in m.StorageInRegulationZone[rz]) >= m.ZonalRegulationDnRequirement[rz,t]
    ...
    def system_reg_up_provided(m,t):
        return (sum(m.RegulationReserveUp[g,t] for g in m.AGC_Generators) + m.SystemRegulationUpShortfall[t])
    model.SystemRegulationUpProvided = Expression(model.TimePeriods, rule=system_reg_up_provided)
    
    def system_reg_dn_provided(m,t):
        return (sum(m.RegulationReserveDn[g,t] for g in m.AGC_Generators) + m.SystemRegulationDnShortfall[t])
    model.SystemRegulationDnProvided = Expression(model.TimePeriods, rule=system_reg_dn_provided)
    
    def enforce_system_regulation_up_requirement_rule(m, t):
        return m.SystemRegulationUpProvided[t] + sum(m.OutputStorage_RegUP[s,t] for s in m.Storage)  >= m.SystemRegulationUpRequirement[t]
    ...
    def enforce_system_regulation_dn_requirement_rule(m, t):
        return (model.SystemRegulationDnProvided[t] + sum(m.InputStorage_RegDOWN[s,t] for s in m.Storage)   >= m.SystemRegulationDnRequirement[t])
...
def spinning_reserves(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, thermal_gen_attrs, storage_in_reserve_zone_getter):
    ...
    model.StorageInSpinningReserveZone = Set(model.SpinningReserveZones, initialize=storage_in_reserve_zone_getter())
    ...
    def zonal_spinning_reserve_provided(m, rz, t):
        return (sum(m.SpinningReserveDispatched[g, t] for g in m.ThermalGeneratorsInSpinningReserveZone[rz]) + m.ZonalSpinningReserveShortfall[rz,t])
    ...
    def enforce_zonal_spinning_reserve_requirement(m, rz, t):
        if regup_reserves and (rz in m.RegulationZones):
            return  m.ZonalSpinningReserveProvided[rz,t] + sum(m.OutputStorage_SP[s,t] for s in m.StorageInSpinningReserveZone[rz])  >= m.ZonalSpinningReserveRequirement[rz, t] 
        else:
            return  m.ZonalSpinningReserveProvided[rz,t] + sum(m.OutputStorage_SP[s,t] for s in m.StorageInSpinningReserveZone[rz])  >= m.ZonalSpinningReserveRequirement[rz, t]
    ...
    def system_spinning_reserve_provided(m,t):
        return (sum(m.SpinningReserveDispatched[g,t] for g in m.ThermalGenerators) + m.SystemSpinningReserveShortfall[t])
    ...
    def enforce_system_spinning_reserve_requirement(m, t):
        if regup_reserves:
            return m.SystemSpinningReserveProvided[t] + sum(m.OutputStorage_SP[s,t] for s in m.Storage) >= m.SystemSpinningReserveRequirement[t]
        else:
            return m.SystemSpinningReserveProvided[t] + sum(m.OutputStorage_SP[s,t] for s in m.Storage) >= m.SystemSpinningReserveRequirement[t]
...
def non_spinning_reserves(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, storage_in_reserve_zone_getter):
    ...
    model.StorageInNonSpinReserveZone = Set(model.NonSpinReserveZones, initialize=storage_in_reserve_zone_getter())
    ...
    def nspin_zonal_reserves_provided(m,rz,t):
        return (sum(m.NonSpinningReserveDispatched[g,t] for g in m.NonSpinGeneratorsInNonSpinZone[rz]) + m.ZonalNonSpinningReserveShortfall[rz,t])
    ...
    def enforce_zonal_non_spinning_reserve_rule(m, rz, t):
        ...
        return m.NonSpinningZonalReservesProvided[rz,t] +\
                + sum(m.OutputStorage_NSP[s,t] for s in m.StorageInNonSpinReserveZone[rz]) \
    ...
    def nspin_reserves_provided(m,t):
        return (sum(m.NonSpinningReserveDispatched[g,t] for g in m.NonSpinGenerators) + m.SystemNonSpinningReserveShortfall[t])
    ...
    def enforce_system_non_spinning_reserve_requirement(m, t):
        return m.SystemNonSpinningReserveProvided[t] +\
                  + sum(m.OutputStorage_NSP[s,t] for s in m.Storage) \
...
def supplemental_reserves(model, zone_initializer_builder, zone_requirement_getter, gens_in_reserve_zone_getter, thermal_gen_attrs, storage_in_reserve_zone_getter):
    ...
    model.StorageInSupplementalReserveZone = Set(model.SupplementalReserveZones, initialize=storage_in_reserve_zone_getter())
    ...
    def operational_zonal_reserves_provided(m,rz,t):
        return (sum(m.SupplementalReserveDispatched[g,t] for g in m.GeneratorsInSupplementalReserveZone[rz]) +
                sum(m.OutputStorage_SUPP[s,t] for s in m.StorageInSupplementalReserveZone[rz]) + 
                m.ZonalSupplementalReserveShortfall[rz,t])
    ...
    def operational_reserves_provided(m,t):
        return sum(m.SupplementalReserveDispatched[g,t] for g in m.ThermalGenerators) \
                + sum(m.OutputStorage_SUPP[s,t] for s in m.Storage) \
    ...
    def enforce_system_supplemental_reserve_requirement(m, t):
        return m.SystemSupplementalReserveProvided[t] >= m.SystemSupplementalReserveRequirement[t] 
